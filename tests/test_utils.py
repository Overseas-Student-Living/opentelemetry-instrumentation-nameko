...
def test_safe_for_serialisation_repr_failure():
    ...
    assert utils.safe_for_serialisation([1, CannotRepr(), 2]) == [
        1,
        "failed to get string representation",
        2,
    ]
...
def test_import_by_path_error_not_a_path():
    ...
    assert str(exc.value) == ("no_dots_in_path doesn't look like a module path")
...
class TestTruncate:
    def test_dont_truncate(self):
        assert utils.truncate("short") == ("short", False)

    def test_truncate_default_length(self):
        assert utils.truncate("x" * 1000) == ("x" * utils.TRUNCATE_MAX_LENGTH, True)
